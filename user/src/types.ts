import type { Map } from 'react-native-kakao-sdk-common';

/** Prompt */
export type Prompt = 'login' | 'cert'
/** 연령대 (한국 나이) */
 ...
 * - SERVICE: 접근권한 관리
 * */
export type ScopeType = 'PRIVACY' | 'SERVICE'
/** 배송 주소 타입 */
export type ShippingAddressType = 'OLD' | 'NEW' | 'UNKNOWN'
...
/** 사용자의 배송지 정보 */
export interface UserShippingAddresses {
...
/** 사용자가 동의한 약관 목록 */
export interface UserServiceTerms {
  /** 동의한 약관 항목 */
...
export interface CertLoginWithKakaoAccountParam {
  prompts?: Prompt[] | null;
  state?: string | null;
  nonce?: string | null;
...
/** 앱투앱 방식 인증 로그인 파라메터 */
export interface CertLoginWithKakaoTalkParam {
  prompts?: Prompt[] | null;
  state?: string | null;
  requestCode: number,
  nonce?: string | null;
...
/** 카카오계정 로그인(웹) 파라메터 */
export interface LoginWithKakaoAccountParam {
  prompts?: Prompt[] | null;
  loginHint?: string | null;
  nonce?: string | null;
  channelPublicIds?: string[] | null;
...
/** 카카오톡 로그인 파라메터 */
export interface LoginWithKakaoTalkParam {
...
/** 동의항목 철회 파라메터 */
export interface RevokeScopesParam {
  /** 동의항목 */
  scopes: string[];
}

/** 동의항목 목록 조회 파라메터 */
export interface ScopesParam {
  /** 동의항목 */
  scopes?: string[] | null;
}

/** 서비스약관 동의 내역 목록 조회 파라메터 */
export interface ServiceTermsParam {
  /** 앱에 사용 설정된 서빗 약관 목록을 함께 요청하려면 'app_service_terms' 값을 지정 */
  extra?: string | null;
}

/** 사용자 배송지 목록 조회 파라메터 */
export interface ShippingAddressesParam {
  /** 주소ID */
