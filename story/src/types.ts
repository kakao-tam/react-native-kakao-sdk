 ...
 * - M: 비공개
 * */
export type StoryPermission = 'A' | 'F' | 'M'
/** 미디어 유형 */
export type MediaType = 'NOT_SUPPORTED' | 'NOTE' | 'PHOTO'
/** 느낌(감정표현)
 ...
 * - CHEER_UP: 힘내요
 * */
export type Emotion = 'UNKNOWN' | 'NOT_DEFINED' | 'LIKE' | 'COOL' | 'HAPPY' | 'SAD' | 'CHEER_UP'
...
/** 스토리 삭제 파라메터 */
export interface DeleteStoryParam {
  /** 포스트 ID */
  id: string;
}

/** 링크정보 파라메터 */
export interface LinkInfoParam {
  url: string;
}

/** 링크 스토리 쓰기 파라메터 */
export interface PostLinkParam {
  /** 콘텐츠 */
  content: string;
  /** 링크 정보 */
  linkInfo: LinkInfo;
...
/** 글 스토리 쓰기 파라메터 */
export interface PostNoteParam {
  /** 콘텐츠 */
  content: string;
  /** 공개 범위 */
  permission: StoryPermission;
...
/** 로컬 이미지 업로드 파라메터 */
export interface StoryUploadParam {
  /** 파일경로 */
  images: string[];
}
